...
$(document).ready(script(localStorage[0] === undefined ? navigator.language : localStorage[0]));
...
function script(lang) {
    ...
    $("#language-list a").on("click", function (e) {
        localStorage[0] = e.target.getAttribute("id");
        location.reload();
    });
    ...
    for (var _i = 0; _i < data.education.length; _i++) {
        ...
        ed_item_img.setAttribute("src", ED[_i].src);
        ed_item_img.setAttribute("alt", ED[_i].alt);
        ed_item_desc.setAttribute("class", "ed-desc");

        ed_item_desc.appendChild(document.createElement("H3"));
        ed_item_desc.lastChild.innerHTML = ED[_i].ed_type;

        ed_item_desc.appendChild(document.createElement("P"));
        ed_item_desc.lastChild.innerHTML = ED[_i].education_desc;

        ed_item.appendChild(ed_item_img);
        ed_item.appendChild(ed_item_desc);
        ed_item.appendChild(document.createElement("HR"));
